...
const arcCentroid = (data, fns) => {
  ...
  var r = ((+fns.innerRadius(data)) + (+fns.outerRadius(data))) / 2,
      a = ((+fns.startAngle(data))  + (+fns.endAngle(data))) / 2 - PI / 2;
  return [Math.cos(a) * r, Math.sin(a) * r];
};


const arcPath = (data, fns) => {
  if(fns === undefined || typeof fns !== 'object') { fns = {}; }
  Object.assign(fns, {
    innerRadius: (d) => { return d.innerRadius || 0; },
    outerRadius: (d) => { return d.outerRadius || 0; },
  ...
  var buffer,
      r,
      r0 = +fns.innerRadius(data),
      r1 = +fns.outerRadius(data),
      a0 = +fns.startAngle(data) - HALF_PI,
      a1 = +fns.endAngle(data) - HALF_PI,
      da = Math.abs(a1 - a0),
      cw = a1 > a0;
  ...
  if (r1 <= EPSILON) {
    ...
    var a01 = a0,
        ...
        da0 = da,
        da1 = da,
        ap = fns.padAngle.apply(this, arguments) / 2,
        padRadius = fns.padRadius(data),
        rp = (ap > EPSILON) && (padRadius ? +padRadius : Math.sqrt(r0 * r0 + r1 * r1)),
        rc = Math.min(Math.abs(r1 - r0) / 2, +fns.cornerRadius(data)),
        rc0 = rc,
        rc1 = rc,
        t0,
        t1;
